...
// Haal afspraken op voor de maand
try {
    $stmt = $pdo->prepare("SELECT * FROM afspraken WHERE MONTH(datum) = :month AND YEAR(datum) = :year");
    $stmt->execute([':month' => date('m'), ':year' => date('Y')]);
